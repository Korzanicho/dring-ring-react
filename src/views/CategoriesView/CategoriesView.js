...
function CategoriesView() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      const defaultSelectedCategories = response.data.filter(
  ...
  const fetchChallenges = async () => {
    try {
      const response = await axios.get(buildApiUrl("challenges"), {
        params: {
  ...
  return (
    <PageContainer className="categories-view mt-3">
      ...
      <div className="categories-view__center text-center">
        ...
        !categoriesData.list.length ? (
          <div>
            <p>Wystąpił błąd!</p>

            <TheButton type="submit" onClick={fetchCategories}>
              Odśwież
            </TheButton>
          </div>
        ) : null}
